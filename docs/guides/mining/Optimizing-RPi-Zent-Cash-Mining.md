...
Compiling the miner could take several minutes to complete. When your miner is ready you need to create a configuration file with your Zent Cash settings. Navigate to your home directory and create a file named config.json.ztc.

```bash
cd ~
sudo nano config.json.ztc
```
...
Create a separate miner start script so that you can easily start the miner and invoke the proper cofiguration file with a single command. I like to name it "minezentcash".

```
sudo nano minezentcash
```

Place the following code into the minezentcash file:

```bash
#!/bin/bash
sudo sysctl -w vm.nr_hugepages=8
~/xmrig/build/xmrig --config=config.json.ztc
```

Make the file executable as a script:

```
sudo chmod +x minezentcash
```
